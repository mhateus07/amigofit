...
- [ ] Frontend: aba Treino básica (form manual + lista + check-in) — **implementado em 2026-09-08** (`TreinoScreen.tsx`, `useWorkoutPlan.ts`, aba nova em `App.tsx`), aguardando validação no iPhone físico
...
- [ ] Frontend: anexar/gravar vídeo por exercício + player inline — **implementado em 2026-09-08** (`expo-video` novo, `NSCameraUsageDescription` adicionada no `app.json`, `prebuild` + `pod install` + remoção do `aps-environment` de novo — mesma pegadinha de sempre), aguardando validação no iPhone físico
